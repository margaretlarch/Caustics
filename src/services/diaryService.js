...
async function generateDailyLog(userId) {
  ...
  const [logsRes, checkinsRes, schedulesRes] = await Promise.all([
    ...
    supabase
      .from('shadow_checkins')
      .select('message, user_response, checked_at')
  ...
  const totalEntries =
    activities.logs.length + activities.checkins.length + activities.schedules.length;

  if (totalEntries === 0) {
    console.log('[diaryService] 今日无活动，写入空日记');
  ...
  try {
    ...
    const entries = [
      ...activities.logs.map(l => ({
        time: new Date(l.created_at).toLocaleTimeString('zh-CN'),
        content: l.content,
        type: 'log',
      })),
      ...activities.checkins.map(c => ({
        time: new Date(c.checked_at).toLocaleTimeString('zh-CN'),
        content: c.message,
        type: 'checkin',
      })),
      ...activities.schedules.map(s => ({
        time: s.completed_at ? new Date(s.completed_at).toLocaleTimeString('zh-CN') : '',
        content: s.title,
        type: 'schedule',
      })),
    ];
